...
def home(request):
    if not request.user.is_superuser and request.user.is_authenticated:
        ...
        context['cuser']=uobj
        context['stories']=UserStory.objects.filter(user_id=request.user.id)
...
def browse_jobs(request):
    ...
    if request.user.is_authenticated:
        cuser = CustomUser.objects.get(user=request.user)
    context['jobs'] = applicable_jobs(None)
    skill_list = Skill.objects.all()
    language_list = CommunicationLanguage.objects.all()
    context['skill_list'] = skill_list
    context['language_list'] = language_list
    if(request.method=='GET'):
        ...
        context['cuser'] = CustomUser.objects.all()
        context['auser'] = User.objects.all()
...
def admin(request):
    if not request.user.is_authenticated:
        ...
        if request.user.is_superuser:
            ...
            context['no_of_jobs'] = 3 #no_of_jobs
            #print(no_of_jobs)
            context['skill_check']=request.GET.get('skill',None)
            context['language_check']=request.GET.get('language',None)
            context['cuser'] = CustomUser.objects.all()
...
def search(request):
    if request.method=='POST':
        ...
        context['skill_check']=request.GET.get('skill',None)
        context['language_check']=request.GET.get('language',None)
        context['cuser'] = CustomUser.objects.filter(bio__icontains = query)
        context['auser'] = User.objects.all()
        return render(request, 'admindashboard.html', context)
...
def view_profile(request,user_id):
    context=dict()
    cuser = CustomUser.objects.get(user=int(user_id))
    context['auser'] = User.objects.get(id=int(user_id))
    context['cuser']=cuser
    context['uskills']=UsersSkill.objects.filter(user_id=int(user_id))
    context['skills']=Skill.objects.all()
    context['languages']=CommunicationLanguage.objects.all()
    context['ulanguages']=UsersCommunicationLanguage.objects.filter(user_id=int(user_id))
    context['stories']=UserStory.objects.filter(user_id=int(user_id))
    return render(request,'view_profile.html',context)

def view_story(request,story_id):
    context=dict()
    story_obj=UserStory.objects.get(id=int(story_id))
    cuser=User.objects.get(id=int(story_obj.user_id))
    context['cuser']=cuser
    context['story']=story_obj
    file_path='media/doc_files/'+story_obj.story_title+str(cuser.username)+'.docx'
...
def save_story(request):
    if request.user.is_authenticated:
        if request.method=='POST':
            ...
            story=request.POST.get('editor')
            story_obj=UserStory(story=story,story_title=story_title,user_id=request.user.id)
            story_obj.save()
            document = Document()
            document.add_heading(story_title, level=1)
            document.add_paragraph(html2text.html2text(story))

            document.save('media/doc_files/'+story_title+str(request.user)+'.docx') 
            file_path='media/doc_files/'+story_title+str(request.user)+'.docx'
            return HttpResponseRedirect(reverse('Portal:index'))
...
def edit_story(request,story_id):
        ...
        context['cuser']=uobj
        context['stories']=UserStory.objects.filter(user_id=request.user.id)
        context['edit_story']=UserStory.objects.get(id=story_id).story
        return render(request,'dashboard.html',context)
